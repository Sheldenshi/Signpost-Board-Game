...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        int x = 0;
        int y = 0;
        for (Place square :start.successors()) {
            if (start.connectable(model.get(square))) {
                if (start.sequenceNum() != 0
                        && (model.get(square).sequenceNum()
                        == start.sequenceNum() + 1)) {
                    return model.get(square);
                }
                connectableCount++;
                x = square.x;
                y = square.y;
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        ...
        int x = 0;
        int y = 0;
        for (Place square : end.predecessors()) {
            if (model.get(square).connectable(end)) {
                counter++;
                x = square.x;
                y = square.y;
            }
