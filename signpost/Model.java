...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        PlaceList[][][] successors = Place.successorCells(width(), height());
        PlaceList[][] predecessors = predecessorCells();
        _solnNumToPlace = new Place[last + 1];
        for (int wid = 0; wid < width(); wid++) {
            for (int hei = 0; hei < height(); hei++) {
                int seqNum = _solution[wid][hei];
                allNumbers.add(seqNum);
                Place temp = pl(wid, hei);
                _solnNumToPlace[seqNum] = temp;
                if (seqNum == 1) {
                    Sq curr =
                            new Sq(wid, hei, seqNum, true,
                                    arrowDirection(wid, hei), 0);
                    curr._successors =
                            successors[wid][hei][arrowDirection(wid, hei)];
                    _board[wid][hei] = curr;
                    _allSquares.add(curr);
                } else if (seqNum == last) {
                    Sq curr =
                            new Sq(wid, hei, seqNum, true,
                                    arrowDirection(wid, hei), 0);
                    curr._predecessors = predecessors[wid][hei];
                    _board[wid][hei] = curr;
                    _allSquares.add(curr);
                } else {
                    Sq curr =
                            new Sq(wid, hei, 0, false,
                                    arrowDirection(wid, hei), -1);
                    curr._successors =
                            successors[wid][hei][arrowDirection(wid, hei)];
                    curr._predecessors = predecessors[wid][hei];
                    _board[wid][hei] = curr;
        ...
        int checker = 1;
        while (checker <= last) {
            if (!allNumbers.contains(checker)) {
    ...
    /** Returns all possiable predecessors. */
    PlaceList[][] predecessorCells() {
        PlaceList[][]P = new PlaceList[width()][height()];
    ...
    Model(Model model) {
        ...
        PlaceList[][][] successors =
                Place.successorCells(_width, _height);
        PlaceList[][] predecessors = predecessorCells();
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                Sq temp = new Sq(model.get(x, y).x, model.get(x, y).y,
                        model.get(x, y).sequenceNum(),
                        model.get(x, y)._hasFixedNum,
                        model.get(x, y).direction(), model.get(x, y).group());
                temp._successors = model.get(x, y)._successors;
                temp._predecessors = model.get(x, y)._predecessors;
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                if (model.get(x, y).successor() != null) {
                    int xOldSuccessor =
                            model.get(x, y).successor().x;
                    int yOldSuccessor =
                            model.get(x, y).successor().y;
                    get(x, y)._successor =
                            get(xOldSuccessor, yOldSuccessor);
                }
                if (model.get(x, y).predecessor() != null) {
                    int xOldPredecessor =
                            model.get(x, y).predecessor().x;
                    int yOldPredecessor =
                            model.get(x, y).predecessor().y;
                    get(x, y)._predecessor =
                            get(xOldPredecessor, yOldPredecessor);
                }
                int xOldHead = model.get(x, y).head().x;
                int yOldHead = model.get(x, y).head().y;
                get(x, y)._head = get(xOldHead, yOldHead);
    ...
    boolean autoconnect() {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                if (get(x, y)._successors != null) {
                    for (Place square : get(x, y)._successors) {
                        if (get(x, y)._sequenceNum
                                == get(square).sequenceNum() - 1) {
                            get(x, y).connect(get(square));
                            madeChange = true;
                        }
                    }
                }
    ...
    final class Sq {
        ...
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() != 0 && this.successor() != null) {
                int number = this.sequenceNum();
                Sq successor = this.successor();
                while (successor != null) {
                    successor._sequenceNum = number + 1;
                    successor = successor.successor();
                    number++;
                }
            } else if (s1.sequenceNum() != 0
                    && s1.predecessor() != null) {
                int number = s1.sequenceNum();
                Sq predecessor = s1.predecessor();
                while (predecessor != null) {
                    predecessor._sequenceNum = number - 1;
                    predecessor = predecessor.predecessor();
                    number--;
                }
            }
            if (s1Numbered && s1.sequenceNum() != 0) {
                releaseGroup(sgroup);
            }
            if (thisNumbered && this.sequenceNum() != 0) {
                int oldGroup = this._head._group;
                releaseGroup(oldGroup);
            }
            if (this.sequenceNum() == 0 && s1.sequenceNum() == 0) {
                this._head._group =
                        joinGroups(this._head._group, s1._head._group);
            }
            Sq updatingHead = s1;
            while (updatingHead != null) {
                updatingHead._head = this.head();
                updatingHead = updatingHead.successor();
            }
            return true;
        }
        /** Disonnect this square to S1, if both are connectable; otherwise do
         *  nothing. Returns true iff this square and S1 were connectable.
         *  Assumes S1 is in the proper arrow direction from this square. */
        void disconnect() {
            Sq next = _successor;
            ...
            if (_sequenceNum == 0) {
                if (this.predecessor() == null
                    ...
                    next._group = -1;
                } else if (this.predecessors() != null
                        && next.successors() != null) {
                    next._group = newGroup();
                }
            } else {
                if (!haveFixedNumberThisCheck(this)) {
                    Sq unfixPredecessors = this;
                    if (this.predecessor() == null) {
                        this._group = -1; this._sequenceNum = 0;
                    } else {
                        int newGroupThis = newGroup();
                        while (unfixPredecessors != null) {
                            unfixPredecessors._sequenceNum = 0;
                            unfixPredecessors.head()._group = newGroupThis;
                            unfixPredecessors = unfixPredecessors.predecessor();
                        }
                    }
                }
                if (!haveFixedNumberNextCheck(next)) {
                    Sq unfixSuccessors = next;
                    if (next.successor() == null) {
                        next._group = -1; next._sequenceNum = 0;
                    } else {
                        int newGroupNext = newGroup();
                        while (unfixSuccessors != null) {
                            unfixSuccessors._sequenceNum = 0;
                            unfixSuccessors._group = newGroupNext;
                            unfixSuccessors = unfixSuccessors.successor();
                        }
                    }
                }
            ...
            while (nextGroup != null) {
                nextGroup._head = next;
                nextGroup = nextGroup.successor();
            }
        }

        /** Return true if next has a fixed number.
         * @return if next has a fixed number at the end of its group
         * @param next - next Sq obj. from disconnect*/
        boolean haveFixedNumberNextCheck(Sq next) {
            boolean haveFixedNumberNext = false;
            Sq successor1 = next;
            while (successor1 != null) {
                if (successor1.hasFixedNum()) {
                    haveFixedNumberNext = true;
                }
                successor1 = successor1.successor();
            }
            return haveFixedNumberNext;
        }

        /** Return true if this has a fixed number.
         * @return if curr has a fixed number at the head of this's group
         * @param curr - this Sq obj. from disconnect*/
        boolean haveFixedNumberThisCheck(Sq curr) {
            boolean haveFixedNumberThis = false;
            Sq predecessor = curr;
            while (predecessor != null) {
                if (predecessor.hasFixedNum()) {
                    haveFixedNumberThis = true;
                }
                predecessor = predecessor.predecessor();
            }
            return haveFixedNumberThis;
        }
